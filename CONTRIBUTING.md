...
Then install the dependencies with `npm ci`. When you have made your changes and are ready to test them, run `npm run build` to compile the TypeScript into JavaScript. Before opening a PR, verify the linter passes with `npm run lint` and the tests pass with `npm run test` - both are required for a PR to be accepted.
